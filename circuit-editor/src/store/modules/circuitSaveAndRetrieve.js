        ...
        if (xmlHttpReq.readyState === 4) {
          if (xmlHttpReq.responseText && xmlHttpReq.status === 200) {
            ...
            for (const [key, value] of Object.entries(jsonObj)) {
              ...
              if (circuitState.version == "1.0" || circuitState.version == "1.1") {
                alertUser = true;
                jsonObj[circuitId] = {
                  "version": "1.2",
                  "circuit_type": "simple",
            ...
            if (alertUser) {
              alert("Unfortunately your circuit has an outdated format. We will try to refrain from introducing backwards incompatible changes in the future. For now, please delete this circuit(s) and create a new a one.");
            }
            return jsonObj;
          } else {
            retries += 1;
            if (retries < 2) {
              ...
              alert('Failed to retrieve your project. Please try again a bit later.');
              return null;
            }
          }
        }
