...
each qubit is measured into a classical bit with index equal to the qubit index of the measure gate.';
    ...
    return "In order to work with the Circuit Editor use the gates pallete on the left to drag & drop gates in the circuit \
            ...
            <br> \
            <b> Switching between gates:</b> \
            <br/> \
            <br/> \
            - you can switch the type of gate that is placed in a circuit via a dialog raised by pressing the <i>'s'</i> or <i>'S'</i> key while clicking the image of the gate.  <br> \
            <br/> \
             <b> Deleting gates:</b> \
            <br/> \
            <br/> \
...
export function retriveControlledGateHelpHtml(gateName) {
  ...
  if (gateName == "u3") {
      return 'An "u3" gate may have any number of controls. The single controlled gate acts on 2 qubits, where one of them acts as a control and the other acts as target. The controlled "u3" gate typically performs a generic (three parameters) unitary operation on the target qubit only when the control qubit is |1⟩ otherwise leaves it unchanged. It is however perfectly possible a setup where the control state(s) are |0⟩, |+⟩, |-⟩, |+i⟩ or |-i⟩ instead of |1⟩. The interpretation of controlled gates as classical controls is useful but has flaws. For example, when one applies a Ctrl+U gate to a qubit that is in an eigenstate of U, the result of the operation is that the control qubit acquires a phase related to the eigenvalue of U. This is the so-called phase kickback effect and is used in many quantum algorithms.';
  } else if (gateName == "u2") {
