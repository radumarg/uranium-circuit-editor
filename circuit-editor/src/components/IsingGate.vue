    ...
    <b-modal ref="modal-dialog" size="sm" centered hide-footer hide-header>
      ...
      <table style="table-layout:fixed;">
        ...
        <tr>
          ...
          <td width="100px" style="padding: 5px;"> 
            <b-form-input min="0" @keyup.enter.native="handleSave()" v-model="thetaNew" placeholder="phi" type="number" id="phi-new" style="width:90px;"></b-form-input>
          </td>
          <td></td>
        </tr>
        <tr>
...
export default {
  ...
  methods: {
    ...mapActions('circuitEditorModule/', ['repositionIsingGateInCircuit']),
    handleSave: function(){
      let qbitOld = this.qbit;
      let qbit2Old = this.qbit2;
      let thetaOld = this.theta;
      let promise = this.repositionIsingGateInCircuit({
        'step': this.step, 
        'qbit': this.qbit, 
        'qbit2': this.qbit2,
        'name': this.name, 
